...
_APP_SCHEMA   = os.getenv('APP_SCHEMA', 'yak')
...
def create_orders_bulk(data: list[OrderIn], db: Session = Depends(get_db), _: str = Depends(_require_admin)):
    ...
    pids = {d.product_id for d in data}
    existing = {r.id for r in db.query(Product.id).filter(Product.id.in_(pids)).all()}
    missing = pids - existing
    if missing:
        raise HTTPException(400, f'존재하지 않는 제품 ID: {sorted(missing)}')
...
def backup_export(db: Session = Depends(get_db), _: str = Depends(_require_admin)):
    wb = Workbook()
    ...
    wb.save(buf)
    buf.seek(0)
    filename = f'{_APP_SCHEMA}_backup_{date_type.today().strftime("%Y%m%d")}.xlsx'
