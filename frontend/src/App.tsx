    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-80 p-6 space-y-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-medium text-slate-500 block mb-1">비밀번호</label>
            <input
              ref={inputRef}
              type="password"
              value={pw}
              ...
              className={`w-full border rounded-lg px-3 py-2.5 text-sm text-center tracking-widest
                ...
                ${error
                  ? 'border-red-300 bg-red-50 focus:ring-red-300 animate-shake'
                  : 'border-slate-200 focus:ring-blue-400'}`}
            />
            {error && <p className="text-xs text-red-500 mt-1 text-center">비밀번호가 틀렸습니다</p>}
          </div>
          <button type="submit"
        ...
        <nav className="flex-1 px-3 py-4">
          ...
          {isAdmin ? (
            <div className="mt-4 space-y-1">
              <p className="px-3 mb-1 text-xs font-semibold text-amber-600 uppercase tracking-wide">
                관리자
              </p>
            ...
            <div className="mt-4 px-3 py-3 bg-slate-50 rounded-lg border border-dashed border-slate-200">
              <p className="text-xs text-slate-400 leading-relaxed">
                ...
                발주 입력 · 입고 관리는<br />관리자 모드에서 가능합니다
              </p>
            </div>
          )}
        </nav>
        ...
        {step === 'select' ? (
          <div className="space-y-3">
            ...
            <button
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-400 mt-0.5">대시보드·현황·내역 조회 가능</p>
              </div>
            </button>
            ...
            <button
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-400 mt-0.5">발주 입력·입고 관리·설정 포함</p>
              </div>
            </button>
          </div>
        ) : (
          <form onSubmit={handleAdmin}
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="font-semibold text-slate-800 text-sm">관리자 비밀번호</p>
            </div>
            <input
              ref={inputRef}
              type="password"
            ...
            <div className="flex gap-2">
              ...
              <button type="submit"
                className="flex-1 py-2.5 bg-amber-500 text-white font-semibold rounded-xl hover:bg-amber-600 transition-colors text-sm">
                입장
              </button>
            </div>
          </form>
...
// ─── 루트 ─────────────────────────────────────────────────
function AppInner() {
  // sessionStorage로 탭 닫으면 초기화, 새로고침은 유지
